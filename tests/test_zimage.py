"""Integration test: ZImage pipeline via /chat endpoint.

Requires a running server (``uvicorn researcher.main:app``) with
IMAGE_BACKEND=zimage in .env.  Run with::

    pytest tests/test_zimage.py -v -s
...
def test_zimage_generates_image(jwt_token):
    """POST /chat with an image prompt returns a generated image tag."""
    resp = requests.post(
        f"{HOST}/chat",
        headers={
    ...
    print(f"\n✅ ZImage test passed. Response excerpt:\n{response_text[:300]}")
